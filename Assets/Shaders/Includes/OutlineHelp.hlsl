...
// reference: https://ameye.dev/notes/edge-detection-outlines/
void Outline_float(float2 UV, float OutlineThickness, float DepthSensitivity, float NormalsSensitivity, float ColorSensitivity, float4 OutlineColor, out float4 Out)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        depthSamples[i] = SHADERGRAPH_SAMPLE_SCENE_DEPTH(UV); // SAMPLE_TEXTURE2D(_CameraDepthTexture, sampler_CameraDepthTexture, uvSamples[i]).r;
        normalSamples[i] = DecodeNormal(SAMPLE_TEXTURE2D(_NormalsBuffer, sampler_point_clamp, UV));// SAMPLE_TEXTURE2D(_CameraDepthNormalsTexture, sampler_CameraDepthNormalsTexture, uvSamples[i]));
